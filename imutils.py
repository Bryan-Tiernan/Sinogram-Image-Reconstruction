...
def getChannel(im, channel='R'):
    ...
    if len(im.shape) < 3:   # Must be a greyscale image, return it unchanged.
        return im
    else:                   # Has a number of channels.
        channel = channel.upper()[0]   # Uppercase 1'st letter.
        if channel == 'B':   chNum = 2
        elif channel == 'G': chNum = 1
        else:                chNum = 0
    chImage = np.zeros(im.shape, dtype = im.dtype)
...
def ghistplot(h,filled=True,barcolour='black',ymax=None,newfig=True):
    ...
    if len(h.shape) != 1:  ## This must be an image, convert it to a histogram.
        h = greyhisto(h)
    if filled:
        pyp.bar(list(range(256)),h,width=1,color=barcolour,linewidth=0)
    else:
...
def makeCLhist(im, maxslope=3.5,maxiter=10,redistribute=False,verbose=True):
    ...
    if verbose: 
        print("Building slope-limited histogram, target slope =", maxslope, end=' ')
